...
async function enviarPedidoCompleto() {
  ...
  // Las líneas del detalle se guardan a precio normal por unidad (sin
  // descuento), porque así se ve más claro en el comprobante. Para que
  // la suma de esas líneas coincida con el total real (que sí aplica
  // la promo 3x1), agregamos una línea aparte de "Descuento" — igual
  // que un recibo real.
  const itemsPayload = grupos.map(g => ({
    producto:        g.nombre,
    masa:            g.masa,
    cantidad:        g.cant,
  ...
  const desglose = _desgloseTrad();
  if (desglose && desglose.lotes > 0) {
    const ahorroPorLote = (PROMO_TRAD.qty * PROMO_TRAD.precioUnidad) - PROMO_TRAD.precio;
    const ahorroTotal   = +(desglose.lotes * ahorroPorLote).toFixed(2);
    itemsPayload.push({
      producto:        `Descuento promo 3×$${PROMO_TRAD.precio.toFixed(2)} (${desglose.lotes} ${desglose.lotes > 1 ? 'grupos' : 'grupo'})`,
      masa:            '',
      cantidad:        desglose.lotes,
      personalizacion: '',
      precio_unitario: -ahorroPorLote,
      subtotal:        -ahorroTotal,
    });
  }
