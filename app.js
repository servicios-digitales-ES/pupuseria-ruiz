...
async function _cargarEstadoEnDrawer() {
  const ultimoId = typeof lsGet === 'function' ? lsGet(LS.ULTIMO_ID) : null;
  ...
  try {
    const res    = await fetchEstadoPedido(ultimoId);
    ...
    contenido.innerHTML = `
      <div style="text-align:center;padding:8px 0 20px;">
        <div style="font-size:.65rem;color:var(--text3);text-transform:uppercase;letter-spacing:.1em;margin-bottom:6px;">Pedido</div>
        <div style="font-size:2rem;font-weight:800;color:var(--accent2);letter-spacing:-.02em;">#${pedido.ID_pedido}</div>
        <div style="font-size:.78rem;color:var(--text3);margin-top:3px;">${pedido.fecha} · ${pedido.hora}</div>
        <div style="font-size:.9rem;font-weight:800;color:${estadoColor};margin-top:14px;padding:8px 16px;background:rgba(255,255,255,.05);border-radius:99px;display:inline-block;">${estadoTexto}</div>
      </div>
      <div style="border-top:1px solid var(--surface2);padding-top:14px;font-size:.85rem;">
        <div style="display:flex;justify-content:space-between;padding:6px 0;color:var(--text);">
          <span style="color:var(--text3);">Cliente</span>
          <span style="font-weight:600;">${pedido.nombre_cliente || '—'}</span>
        </div>
        <div style="display:flex;justify-content:space-between;padding:6px 0;color:var(--text);">
          <span style="color:var(--text3);">Entrega</span>
          <span style="font-weight:600;">${pedido.tipo_entrega || '—'}</span>
        </div>
      </div>
      <a href="estado.html?id=${pedido.ID_pedido}" class="btn-ghost btn-full" style="margin-top:16px;text-align:center;justify-content:center;display:flex;">
        Ver detalle completo →
      </a>`;
  } catch(e) {
    if (contenido) contenido.innerHTML = `
      <div style="text-align:center;padding:20px;color:var(--text3);font-size:.85rem;">
        No se pudo cargar el estado.<br>
...
function _mostrarExito({ idPedido, total }) {
  document.getElementById('exito-num-pedido').textContent = idPedido ? `#${idPedido}` : 'Confirmado';
